...
export async function fetchEmployeeEmploymentTypeData(filters: Filters): Promise<EmployeeTypeEntry[]> {
    return extractEmployeeData<EmployeeTypeEntry>(filters, (_, filteredEmployees) => {
        const employmentTypeCount: Record<string, number> = {};
        filteredEmployees.forEach(emp => {
            employmentTypeCount[emp.employmentType] = (employmentTypeCount[emp.employmentType] || 0) + 1;
        });
        return Object.entries(employmentTypeCount).map(([employmentType, count]) => ({
...
function filterDates(dates: string[], dateRange: DateRange | undefined, fetchLatestDateOnly: boolean): string[] {
    if(fetchLatestDateOnly) {
